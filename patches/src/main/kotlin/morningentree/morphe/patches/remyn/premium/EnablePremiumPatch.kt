...
import morningentree.morphe.patches.remyn.shared.Constants
import morningentree.morphe.util.returnEarly

@Suppress("unused")
val enablePremiumPatch = bytecodePatch(
    name = "Enable Premium",
    description = "Unlocks Remyn Premium. Use With Spoof Install Source.",
) {
    compatibleWith(Constants.COMPATIBILITY)

    execute {
        // Remyn is Flutter: the premium decision is in AOT Dart, but billing runs
        ...
        // gate pass regardless of the entitlement id. If Remyn instead checks a
        // specific id via `entitlements.active["<id>"]`, change ENTITLEMENT_ID below to
        // that id (confirm it from the arm64 split's libapp.so or by runtime testing).
