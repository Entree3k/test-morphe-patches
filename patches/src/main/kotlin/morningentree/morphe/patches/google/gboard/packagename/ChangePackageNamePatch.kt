...
/**
 * Installs the patched Gboard as a separate clone by changing its package name, so it lives
 * alongside the stock, Google-signed Gboard instead of failing with
 * INSTALL_FAILED_UPDATE_INCOMPATIBLE. Off by default.
 *
 * After installing, enable the clone in Settings -> System -> Languages & input -> On-screen
 * keyboards. Note: renaming an input method's package is inherently a bit risky; if the clone
 * misbehaves, prefer uninstalling stock Gboard for your user
 * (`adb uninstall --user 0 com.google.android.inputmethod.latin`) and installing without this patch.
 */
@Suppress("unused")
