...
import morningentree.morphe.patches.recorder.pairip.disablePairipStartupPatch
...
val removeDeviceRestrictionsPatch = bytecodePatch(
    name = "Remove device restrictions (dev)",
    description = "Removes restrictions from using the app on any device. Requires mounting patched " +
        "app over original.",
) {
    compatibleWith(Constants.COMPATIBILITY)

    // Neutralize the Java-side Pairip signature/license checks and, crucially for this app, the
    // native startup integrity program (StartupLauncher.launch) that self-destructs in
    // libpairipcore.so on a re-signed / non-mounted install.
    dependsOn(disablePairipPatch, disablePairipStartupPatch)
